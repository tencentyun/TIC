...
struct TICClassroomOption
{
	...
	std::string	ntpServer	= "time1.cloud.tencent.com";		//ntp服务器;进房成功后从ntp服务器获取服务器时间戳作为课后录制的对时信息,默认使用time1.cloud.tencent.com;为保证对时信息的高度一致,建议各端使用相同对时地址;
...
public:
	 ...
	 *********************************************************************************************/
	 ...
	 /**
	  * 发送离线录制对时信息
	  * @note TIC内部进房成功后会自动发送离线录制对时信息,如果发送失败回调onTICSendOfflineRecordInfo接口且code!=0,可以调用此接口触发重试;
	  */
