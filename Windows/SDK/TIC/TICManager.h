...
#ifdef TIC_EXPORTS  // TIC导出为DLL,在DLL项目属性中增加预处理器TIC_EXPORTS即可
...
enum TICClassScene {
  ...
  TIC_CLASS_SCENE_LIVE =
      1,  // 直播模式，支持1000人以上场景，会增加600ms左右延时
};
...
struct TICMessageListener {
   ...
   * @note
   * 所有收到的消息都会在此回调进行通知，包括前面已经封装的文本和自定义消息;
   */
...
struct TICEventListener {
   ...
   * @param desc				错误信息;
   * @note
   * 进房成功后,TIC会自动发送离线录制需要的对时信息;只有成功发送对时信息的课堂才能进行课后离线录制;
   * 注: 可能在子线程中执行此回调;
   */
...
class TIC_API TICManager {
 public:
  ...
  /**
   * 创建课堂
   * @param classId			课堂ID，32位整型，取值范围[1,
   * 4294967294]，由业务生成和维护
   * @param callback			回调
   */
   ...
   * @param callback			回调
   * @note 房间人数超过200人，建议启用大房间模式，详细配置请参考
   ...
   * @param listener			回调
   * @note
   * 不能在TICMessageListener的回调中调用AddMessageListener()\RemoveMessageListener(),否则死锁;
   */
   ...
   * @param listener			回调
   * @note
   * 不能在TICEventListener的回调中调用AddEventListener()\RemoveEventListener(),否则死锁;
   */
   ...
   * @param listener			回调
   * @note
   * 不能在TICIMStatusListener的回调中调用AddStatusListener()\RemoveStatusListener(),否则死锁;
   */
   ...
   * 发送离线录制对时信息
   * @note
   * TIC内部进房成功后会自动发送离线录制对时信息,如果发送失败回调onTICSendOfflineRecordInfo接口且code!=0,可以调用此接口触发重试;
   */
