...
void HttpHeaders::SetHeader(const std::wstring &key, const std::wstring &value)
{
...
void HttpHeaders::SetContentType(std::wstring contentType /*= L"application/json"*/)
{
...
DWORD HttpRequest::errorCode() const
{
...
void HttpRequest::init(const std::wstring& userAgent, const std::wstring& url, const std::wstring& method, HttpAsynCallback asynCallback /*= nullptr*/)
{
	...
	URL_COMPONENTS_ANOTHER url_comp = { 0 };
	...
	//hConnect
	m_hConnect = ::WinHttpConnect(m_hSession, hostName.c_str(), url_comp.nPort, 0);
	if (!m_hConnect)
	{
		m_dwErrorCode = GetLastError();
		return;
...
void HttpRequest::receiveResponseHeaders(DWORD& dwStatusCode, HttpHeaders& rspHeaders)
{
	if (m_dwErrorCode != 0) return;
	...
	BOOL bRet = ::WinHttpQueryHeaders(
		...
		&dwHeaderSize,
		NULL);
	if (!(FALSE == bRet && GetLastError() == ERROR_INSUFFICIENT_BUFFER && dwHeaderSize > 0))
	{
		m_dwErrorCode = GetLastError();
		return;
	}
...
int HttpRequest::queryDataAvailable()
{
	...
	if (FALSE == ::WinHttpQueryDataAvailable(m_hRequest, m_asynCallback ? NULL : &availableSize))
	{
		m_dwErrorCode = GetLastError();
		return -1;
	}
...
void HttpRequest::readData(void* buffer, DWORD dwBytesToRead, DWORD* pdwBytesRead /*= nullptr*/)
{
	if (m_dwErrorCode != 0) return;

	if (FALSE == ::WinHttpReadData(m_hRequest, buffer, dwBytesToRead, pdwBytesRead))
	{
		m_dwErrorCode = GetLastError();
	}
}
...
void CALLBACK HttpRequest::_onWinHttpStatusCallback(HINTERNET hInternet, DWORD_PTR dwContext, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
{
	...
	{
		pThis->m_dwErrorCode = reinterpret_cast<LPWINHTTP_ASYNC_RESULT>(lpvStatusInformation)->dwError;
		if(pThis->m_asynCallback) pThis->m_asynCallback(HttpAsynCBType::RequestError, nullptr);
		break;
	}
	case WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE:
	{
		if (pThis->m_asynCallback) pThis->m_asynCallback(HttpAsynCBType::SendRequestComplete, nullptr);
		break;
	}
	case WINHTTP_CALLBACK_STATUS_WRITE_COMPLETE:
	{
		...
		if (pThis->m_asynCallback) pThis->m_asynCallback(HttpAsynCBType::WriteComplete, &param);
		break;
	}
	case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE:
	{
		if (pThis->m_asynCallback) pThis->m_asynCallback(HttpAsynCBType::HeadersAvailable, nullptr);
		break;
	}
	case WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE:
	{
		...
		if (pThis->m_asynCallback) pThis->m_asynCallback(HttpAsynCBType::DataAvailable, &param);
		break;
	}
	case WINHTTP_CALLBACK_STATUS_READ_COMPLETE:
	{
		...
		param.pReadBuffer = (char*)lpvStatusInformation;
		if (pThis->m_asynCallback) pThis->m_asynCallback(HttpAsynCBType::ReadComplete, &param);
		break;
	}
...
int HttpClient::get(const std::wstring& url, std::string& rspBody, const HttpHeaders* reqHeaders /*= nullptr*/, HttpHeaders* rspHeaders /*= nullptr*/, HttpProgressCallback progressCB /*= nullptr*/)
{
	...
	request.setTimeouts(m_nResolveTimeout, m_nConnectTimeout, m_nSendTimeout, m_nReceiveTimeout);
	...
	DWORD dwRspBodySize = static_cast<DWORD>(theRspHeaders.GetHeaderAsUInt64(L"Content-Length"));
	...
	while (true)
	{
		...
		if (dwRspBodySize == 0)
		{
			std::unique_ptr<char> ptr = std::unique_ptr<char>(new char[nAvailable+1]);
		...
		if (progressCB) progressCB(HttpAction::Response, dwReceivedSize, dwRspBodySize);
	}

	if (m_bEnableEncodeConvert) convertRespEncode(theRspHeaders, rspBody, rspBody);
...
int HttpClient::post(const std::wstring& url, const std::string& reqBody, std::string& rspBody, const HttpHeaders* reqHeaders /*= nullptr*/, HttpHeaders* rspHeaders /*= nullptr*/, HttpProgressCallback progressCB /*= nullptr*/)
{
	DWORD dwStatusCode = 0;
	HttpHeaders tempRspHeaders_;
	HttpHeaders& theRspHeaders = rspHeaders ? (*rspHeaders) : tempRspHeaders_;
	...
	DWORD dwFragSize = dwSendTotalSize > m_skMaxFragSize ? m_skMaxFragSize : dwSendTotalSize;
	DWORD sentSize = 0;
	if (progressCB && dwSendTotalSize > 0) progressCB(HttpAction::Request, 0, dwSendTotalSize);
...
int HttpClient::download(const std::wstring& url, const std::string& file, HttpProgressCallback progressCB /*= nullptr*/, const HttpHeaders* reqHeaders /*= nullptr*/, HttpHeaders* rspHeaders /*= nullptr*/)
{
	DWORD dwStatusCode = 0;
	HttpHeaders tempRspHeaders_;
	HttpHeaders& theRspHeaders = rspHeaders ? (*rspHeaders) : tempRspHeaders_;
...
void HttpClient::asynDownload(const std::wstring& url, const std::string& file, HttpDownloadComplete completeCB, HttpProgressCallback progressCB /*= nullptr*/, const HttpHeaders* reqHeaders /*= nullptr*/)
{
	HttpRequestInfo* requestInfo = new HttpRequestInfo();
	if ((!requestInfo) || (!requestInfo->request))
	{
...
void HttpClient::convertRespEncode(const HttpHeaders& headers, const std::string& src, std::string& dst)
{
	...
	if ((::wcsstr(contentType.c_str(), L"text") || ::wcsstr(contentType.c_str(), L"json")))
	{
...
std::wstring HttpClient::a2w(const std::string &str, unsigned int codePage /*= CP_ACP*/)
{
