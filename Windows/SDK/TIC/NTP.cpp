...
const static uint64_t OFFSET_1900_TO_1970 = ((365LL * 70LL) + 17LL) * 24LL * 60LL * 60LL; //70 years plus 17 leap days;
...
void NTP::getNTPServerTime(const std::string& ntpServer, int nRetry, NTPCallback cb)
{
	WSADATA wsaData;
	...
	setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (const char*)&nRecvTimeOut, sizeof(nRecvTimeOut));
	...
	sockaddr_in addr = *(sockaddr_in *)addrRes->ai_addr;
	...
	std::thread th([=]() {
		...
		for (int i = 0; i < nRetry; ++i)
		{
			...
			data.Mode = 3;
			int64_t nT1 = std::chrono::system_clock::now().time_since_epoch().count() * _XTIME_NSECS_PER_TICK / 1000000;

			int nRet = sendto(sock, (const char*)&data, sizeof(data), 0, (SOCKADDR *)&addr, sizeof(addr));
			...
			nRet = recvfrom(sock, (char*)&recvData, sizeof(recvData), 0, &addrFrom, &nAddrFromLen);
			if (nRet == SOCKET_ERROR)
			{
				serverTime = 0;
			...
			int64_t nT4 = std::chrono::system_clock::now().time_since_epoch().count() * _XTIME_NSECS_PER_TICK / 1000000;
...
{
	uint64_t now = std::chrono::system_clock::now().time_since_epoch().count() * _XTIME_NSECS_PER_TICK / 1000000; //毫秒
	uint64_t seconds = now / 1000 + OFFSET_1900_TO_1970;
	uint64_t fraction = static_cast<uint64_t>((now % 1000) / 1000.0 * 0x100000000L);
...
uint64_t NTP::_NTPTime2UTCTime(uint64_t ntpTime)
{
	...
	uint64_t millsecond = static_cast<uint64_t>(static_cast<double>(ntpTime & 0xFFFFFFFF) / 0x100000000L * 1000);
