...
ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_BRUSH_THIN,
          &CDrawTabDlg::OnNMCustomdrawSliderBrushThin)
ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_TEXT_SIZE,
          &CDrawTabDlg::OnNMCustomdrawSliderTextSize)
...
ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_SCALE,
          &CDrawTabDlg::OnNMCustomdrawSliderScale)
END_MESSAGE_MAP()
...
void CDrawTabDlg::Init() {
  ...
  chkDrawEnable_.SetCheck(boardCtrl->IsDrawEnable() ? BST_CHECKED
                                                    : BST_UNCHECKED);

  TEduBoardColor brushColor = boardCtrl->GetBrushColor();
  btnBrushColor_.SetFaceColor(
      RGB(brushColor.red, brushColor.green, brushColor.blue), true);
  TEduBoardColor backColor = boardCtrl->GetBackgroundColor();
  btnBackColor_.SetFaceColor(
      RGB(backColor.red, backColor.green, backColor.blue), true);
  TEduBoardColor textColor = boardCtrl->GetTextColor();
  btnTextColor_.SetFaceColor(
      RGB(textColor.red, textColor.green, textColor.blue), true);
...
void CDrawTabDlg::OnBnClickedBtnBkImage() {
  ...
  if (boardCtrl) {
    ...
    if (dlgFile.DoModal() == IDOK) {
      boardCtrl->SetBackgroundImage(
          w2a(dlgFile.GetPathName().GetString(), CP_UTF8).c_str(),
...
void CDrawTabDlg::OnNMCustomdrawSliderBrushThin(NMHDR *pNMHDR,
                                                LRESULT *pResult) {
...
void CDrawTabDlg::OnNMCustomdrawSliderTextSize(NMHDR *pNMHDR,
                                               LRESULT *pResult) {
  LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
  auto *boardCtrl = TICManager::GetInstance().GetBoardController();
  if (boardCtrl) {
...
ON_BN_CLICKED(IDC_BTN_SYNC_AND_RELOAD,
              &CBoardTabDlg::OnBnClickedBtnSyncAndReload)
END_MESSAGE_MAP()
...
void CBoardTabDlg::UpdateBoardList() {
  ...
  if (boardCtrl) {
    ...
    TEduBoardStringList *boardList =
        boardCtrl->GetFileBoardList(fileId.c_str());
...
void CBoardTabDlg::OnCbnSelchangeComboFitMode() {
  auto *boardCtrl = TICManager::GetInstance().GetBoardController();
  if (boardCtrl) {
    boardCtrl->SetBoardContentFitMode(
        (TEduBoardContentFitMode)(comboFitMode_.GetCurSel()));
  }
}
...
void CFileTabDlg::UpdateFileList() {
  ...
  if (boardCtrl) {
    ...
    for (uint32_t i = 0; i < fileList->GetCount(); ++i) {
      ...
      listFile_.SetItemText(i, 2,
                            (std::to_wstring(fileInfo.pageIndex + 1) + _T("/") +
                             std::to_wstring(fileInfo.pageCount))
                                .c_str());
      if (curFileId == fileInfo.fileId) {  // 选中当前文件
        listFile_.SetItemState(i, LVIS_SELECTED | LVIS_FOCUSED,
...
void CFileTabDlg::OnBnClickedBtnAddFile() {
  ...
  if (boardCtrl) {
    CFileDialog dlgFile(TRUE, NULL, NULL, OFN_HIDEREADONLY,
                        _T("All Files (*.*)|*.ppt;*.pptx;*.doc;*.docx;*.pdf||"),
                        NULL);
    if (dlgFile.DoModal() == IDOK) {
      ...
      boardCtrl->ApplyFileTranscode(
          w2a(dlgFile.GetPathName().GetString(), CP_UTF8).c_str(), config);
    }
  }
}
...
CBoardDlg::CBoardDlg(CWnd *pParent)
    : CDialogEx(IDD_BOARD_DIALOG, pParent), histroySync_(false) {
...
void CBoardDlg::Init() {
  ...
  if (boardCtrl) {
    ...
    // 更改白板大小和位置
    boardCtrl->SetBoardRenderViewPos(0, 0, clientRect.right - clientRect.left,
...
BOOL CBoardDlg::OnInitDialog() {
  ...
  imageList_.Create(ThumpWidth, ThumpHeight, ILC_COLOR24, 0,
                    0);  // 创建图像序列CImageList对象
  listThumb_.SetExtendedStyle(listThumb_.GetExtendedStyle() |
...
void CBoardDlg::onTEBAddBoard(const TEduBoardStringList *boardList,
                              const char *fileId) {
  if (histroySync_) boardTabDlg_.UpdateBoardList();
}

void CBoardDlg::onTEBDeleteBoard(const TEduBoardStringList *boardList,
                                 const char *fileId) {
  if (histroySync_) boardTabDlg_.UpdateBoardList();
}
...
void CBoardDlg::onTEBFileTranscodeProgress(
    const char *path, const char *errorCode, const char *errorMsg,
  ...
  if (result.status == TEDU_BOARD_FILE_TRANSCODE_FINISHED) {  // 转码完成
    ...
    boardCtrl->AddTranscodeFile(
        result);  // 实际执行添加操作，添加完成后，到onTEBAddTranscodeFile()回调;
  } else {        // 转码中
    printf("转码进度: %.02lf%%\n", result.progress);
  }
}
...
ON_NOTIFY(LVN_ITEMCHANGED, IDC_LIST_LOCAL_RECORD,
          &CRecordDlg::OnLvnItemchangedListLocalRecord)
ON_BN_CLICKED(IDC_BTN_INIT, &CRecordDlg::OnBnClickedBtnInit)
ON_BN_CLICKED(IDC_BTN_EXIT, &CRecordDlg::OnBnClickedBtnExit)
ON_BN_CLICKED(IDC_BTN_REFRESSH_RESULT,
              &CRecordDlg::OnBnClickedBtnRefresshResult)
ON_NOTIFY(NM_DBLCLK, IDC_LIST_LOCAL_RECORD,
          &CRecordDlg::OnNMDbClkListRecordFile)
...
void CRecordDlg::initRecord(int appid, const std::string &user,
  ...
  mLocalRecorder->init(
      auth, [this, weakSelf](TICModule module, int code, const char *desc) {
...
void CRecordDlg::exitRecord() {
  std::weak_ptr<CRecordDlg> weakSelf = this->shared_from_this();
  mLocalRecorder->exit(
      [this, weakSelf](TICModule module, int code, const char *desc) {
        std::shared_ptr<CRecordDlg> self = weakSelf.lock();
        if (!self) return;
...
void CRecordDlg::startRecord() {
  ...
  mLocalRecorder->startLocalRecord(
      para, "E:\\test\\test.flv",
      [this, weakSelf](TICModule module, int code, const char *desc) {
        std::shared_ptr<CRecordDlg> self = weakSelf.lock();
        if (!self) return;
...
void CRecordDlg::stopRecord() {
  std::weak_ptr<CRecordDlg> weakSelf = this->shared_from_this();
  mLocalRecorder->stopLocalRecord(
      [this, weakSelf](TICModule module, int code, const char *desc) {
        std::shared_ptr<CRecordDlg> self = weakSelf.lock();
        if (!self) return;
...
void CRecordDlg::pauseRecord() {
  std::weak_ptr<CRecordDlg> weakSelf = this->shared_from_this();
  mLocalRecorder->pauseLocalRecord(
      [this, weakSelf](TICModule module, int code, const char *desc) {
        std::shared_ptr<CRecordDlg> self = weakSelf.lock();
        if (!self) return;
...
void CRecordDlg::resumeRecord() {
  std::weak_ptr<CRecordDlg> weakSelf = this->shared_from_this();
  mLocalRecorder->resumeLocalRecord(
      [this, weakSelf](TICModule module, int code, const char *desc) {
        std::shared_ptr<CRecordDlg> self = weakSelf.lock();
        if (!self) return;
...
void CRecordDlg::getRecord() {
  ...
  const RecordKey key = RecordKey(appid, theApp.getClassId(), std::string(""),
                                  std::string(""), 0, 40);

  TEduRecordAuthParam auth(appid, userid, theApp.getUserSig());

  std::weak_ptr<CRecordDlg> weakSelf = this->shared_from_this();
  mLocalRecorder->getRecordResult(
      auth, key,
      [this, weakSelf](TICModule module, int code, const char *desc) {
        std::shared_ptr<CRecordDlg> self = weakSelf.lock();
        if (!self) return;
...
void CRecordDlg::OnLvnItemchangedListLocalRecord(NMHDR *pNMHDR,
                                                 LRESULT *pResult) {
  LPNMLISTVIEW pNMLV = reinterpret_cast<LPNMLISTVIEW>(pNMHDR);
  *pResult = 0;
}
...
void CRecordDlg::OnBnClickedBtnExit() { exitRecord(); }
...
void CRecordDlg::OnBnClickedBtnRefresshResult() { getRecord(); }
      ...
      if (record_info_list.isArray()) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (record.isMember("VideoOutputDuration")) {
            info.VideoOutputDuration =
                record["VideoOutputDuration"].asLargestUInt();
          }
...
void CRecordDlg::refreshRecordInfo() {
  ...
  for (uint32_t i = 0; i < size; ++i) {
    ...
    mListRecord.SetItemText(
        i, 0, a2w(fileInfo.VideoOutputUrl.c_str(), CP_UTF8).c_str());
    mListRecord.SetItemText(i, 1,
                            a2w(fileInfo.UserId.c_str(), CP_UTF8).c_str());
    mListRecord.SetItemText(
        i, 2,
        a2w(std::to_string(fileInfo.VideoOutputDuration / 1000).c_str(),
            CP_UTF8)
            .c_str());
  }
  if (size > 0) {  // 选中当前文件
    mListRecord.SetItemState(0, LVIS_SELECTED | LVIS_FOCUSED,
                             LVIS_SELECTED | LVIS_FOCUSED);
...
void CRecordDlg::OnBnClickedBtnStartRecord() { startRecord(); }

void CRecordDlg::OnBnClickedBtnStopRecord() { stopRecord(); }

void CRecordDlg::OnBnClickedBtnPauseRecord() { pauseRecord(); }

void CRecordDlg::OnBnClickedBtnResumeRecord() { resumeRecord(); }
...
void CBoardTabDlg::OnBnClickedBtnSnapshot() {
  ...
  std::string imagePath =
      w2a(exeDir + L"\\snapshots\\" +
              a2w(boardCtrl->GetCurrentBoard(), CP_UTF8) + L".png",
          CP_UTF8);
