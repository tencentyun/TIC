...
std::string i2s(int num) { return std::to_string((long long)num); }

void showErrorMsg(TICModule module, int code, const char *desc) {
  CString str;
  str.Format(_T("模块: %d 错误码: %d 错误信息: %s"), module, code,
             a2w(desc).c_str());
...
void stretchImage(CImage *pImage, CImage *ResultImage, int outWidth,
  ...
  // 当Destination 比较小的時候,会根据Destination DC 上的 Stretch Blt mode
  // 決定是否要保留被刪除點的資訊
  ...
  StretchBlt(*ResultImageDC, 0, 0, outWidth, outHeight, *pImageDC, 0, 0,
             pImage->GetWidth(), pImage->GetHeight(), SRCCOPY);
...
static std::string createLocalPath(std::string strImgUrl) {
  ...
  return std::string(folderPath) + fileName +
         strImgUrl.substr(strImgUrl.rfind('.'));
}
...
std::string savePic(std::string strImgUrl) {
  ...
  hInternet =
      InternetOpen(_T("TIC_DEMO"), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
  if (hInternet == NULL) {
    return "";
  }

  HINTERNET hUrl;
  hUrl = InternetOpenUrl(hInternet, a2w(strImgUrl).c_str(), NULL, 0,
  ...
  DWORD written;
  HANDLE hFile;
  hFile = CreateFile(a2w(path).c_str(), GENERIC_WRITE, 0, 0, CREATE_ALWAYS,
