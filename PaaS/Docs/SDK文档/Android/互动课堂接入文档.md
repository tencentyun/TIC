...
```java
// board 白板库
compile ('com.tencent.teduboard:TEduBoardSdk:2.4.0.292')
//如果已有tbs版本
//compile ('com.tencent.teduboard:TEduBoardSdkNoTbs:2.4.0.292')

//Trtc 音视频库，如果使用腾讯的云视频 Trtc，则需要加上
compile 'com.tencent.liteav:LiteAVSDK_TRTC:6.9.8321'

//imsdk 消息通信库，如果使用腾讯的即时通信 Tim，则需要加上
compile 'com.tencent.imsdk:imsdk:4.6.56'
```
...
2. 退出课堂比较简单，直接调用  `quitClassroom `接口。注意其中 `clearBoard` 参数，如果置 true，则会将白板的所有数据清空，在下次重新进入此课堂，是空的白板，请慎重使用。在退出课堂后，将会调用白板的接口uninit，销毁白板并结束计费。(注意使用最新的TICSDK)
```java
    //一般是不要清除数据，请慎重使用
