...
Page({
  ...
  onClick() {
    ...
    var url = `/pages/tic/room_webview/room/room?sdkAppId=${this.data.sdkAppId}&identifier=${TEST_ACCOUNT.users[this.data.index]['userId']}&userSig=${TEST_ACCOUNT.users[this.data.index]['userToken']}&roomID=${this.data.roomID}&role=${this.data.role}`;
    wx.navigateTo({
      url: url
    });
  }
